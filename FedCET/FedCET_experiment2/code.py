...
####################################################################
#stochastic
A=np.zeros((iter_update,N,n))
for a in range(0,iter):
    for b in range(0,N):
        A[a][b]=np.random.normal(0, 1, size=(n))
...
for t in range(2,iter_update-5):  
    for i in range(0,N):
        if t%tau==0:
            VALUE=0*X_my1[t,i]
            for j in range(0,N):
                g1=4*X_my1[t,j]-2*B[j]+A[t,j]  
                g2=4*X_my1[t-1,j]-2*B[j]+A[t-1,j] 
...
for t in range(0,iter):
    for i in range(0,N):
        ...
        C_PLUS[i]=4*X2-2*B[i]+np.random.normal(0, 1, size=(n))
        Delta_Y2[i]=Y2[i]-X2
...
for t in range(0,iter):
    ...
    for i in range(0,N):  
        X_my3[i]=ave_X_my3
        GG_FITST_my3=4*X_my3[i]-2*B[i]+np.random.normal(0, 1, size=(n))   # np.dot(np.dot(np.transpose(A[i]),A[i]),X_my3[i])-np.dot(np.transpose(A[i]),b[i])
        
        for k in range(0,tau):
            X_my3[i]=X_my3[i]-step_size3*(GG_AVG_my3-GG_FITST_my3+(4*X_my3[i]-2*B[i]+A[t,i]))
...
np.savetxt('Result_FedCET_TAU6.txt',Result_FedCET,fmt="%f",delimiter=" ") 
np.savetxt('Result_FedTrack_TAU6.txt',Result_FedTrack,fmt="%f",delimiter=" ")
np.savetxt('Result_SCAFFOLD_TAU6.txt',Result_SCAFFOLD,fmt="%f",delimiter=" ")