...
##########################################################
    ...
    DSCMD=np.transpose(readFile('Result_FedCET_TAU6.txt'))
    DSCMDD=np.transpose(readFile('Result_FedTrack_TAU6.txt'))
    DSCMDDD=np.transpose(readFile('Result_SCAFFOLD_TAU6.txt'))
...
##########################################################
 ...
 #   plt.semilogy(x,y,label='FedCET', color='y',linewidth=2.4,linestyle='solid')
    ...
    plt.xlabel(r'$k^{th}$ Information Transmission',fontsize=13)
    plt.ylabel('Convergence Errors',fontsize=13)
    
    
    
    plt.savefig(fname="stochastic_different_algorithm_TAU6.jpg",format="jpg", bbox_inches='tight')
    plt.savefig(fname="stochastic_different_algorithm_TAU6.pdf",format="pdf", bbox_inches='tight')
    plt.show() 
